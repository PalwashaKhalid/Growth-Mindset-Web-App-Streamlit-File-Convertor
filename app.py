...
if uploaded_files:
    for file in uploaded_files:
        ...
        #  Display about the name of file,
        st.write(f"File Name: {file.name}")
        st.write(f"File Size: {file.size/1024}")

        # Show 5 rowa of our data frame as df
        ...
        if st.button(f"Convert {file.name}"):
            ...
            # Download Button:
            st.download_button(
                label= f"⬇️ Download {file.name} as {conversion_type}", data= buffer, file_name=file_name, mime=mime_type
            )
